...
export function useCourses() {
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated) {
      console.log('User not authenticated, clearing courses');
      setCourses([]);
      setIsLoading(false);
      setError(null);
      return;
    ...
    if (!user?.id) {
      console.log('No user ID available, cannot fetch courses');
      setCourses([]);
      setIsLoading(false);
...
export function useSchools() {
  ...
  useEffect(() => {
    // Don't fetch if still loading auth or not authenticated
    if (authLoading) {
      console.log('Auth still loading, waiting for schools...');
      return;
    }
    
    if (!isAuthenticated) {
      console.log('User not authenticated, clearing schools');
      setSchools([]);
      setIsLoading(false);
      setError(null);
      return;
    ...
    if (!user?.id) {
      console.log('No user ID available, cannot fetch schools');
      setSchools([]);
      setIsLoading(false);
      setError(new Error('No user ID available'));
      return;
    ...
    fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/users/${user.id}/orgs`)
      ...
      .then(data => {
        ...
        console.log('Schools fetched successfully:', formattedSchools.length);
