...
async def init_db():
    ...
    async with get_new_db_connection() as conn:
        ...
        try:
            ...
            # Close the connection before attempting to delete the db
            await cursor.close()
            await conn.close()
            try:
                os.remove(sqlite_db_path)
            except PermissionError:
                # If we can't remove the file, just log the error and continue
                print(f"Warning: Could not remove database file: {sqlite_db_path}")
            raise exception
